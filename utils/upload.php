...
function upload_files() {
   ...
   $upfile = $_SERVER['DOCUMENT_ROOT'].'/products_ORM_2/media/'.$_FILES['file']['name'];
    if (is_uploaded_file($_FILES['file']['tmp_name'])){
        if (is_file($_FILES['file']['tmp_name'])) {
           ...
           $upfile = $_SERVER['DOCUMENT_ROOT'].'/products_ORM_2/media/'.$nombreFichero;
        }else{
                $error .=   "Invalid File...";
        }
    }
    ...
    if ($error == "") {
        ...
        if($_FILES['file']['error'] !== 0) { //Assignarem a l'us default-avatar
            $upfile = $_SERVER['DOCUMENT_ROOT'].'/products_ORM_2/media/'.$_FILES['avatar']['name'];
            return $return=array('resultado'=>true,'error'=>$error,'datos'=>$upfile);
        }
    }else{
        return $return=array('resultado'=>false,'error'=>$error,'datos'=>"");
...
function remove_files(){
	$name =$_POST["filename"];
	if(file_exists($_SERVER['DOCUMENT_ROOT'].'/products_ORM_2/media/'.$_SESSION["nombreFichero"])){
